...
import { ArrowLeft, TrendingUp, Target, ChevronUp, FileText, BookOpen, CreditCard, Layers } from "lucide-react";
import { supabase, type Account, type TierUpgradeRequest, type LoanApplication, type ChequeRequest, type VirtualCardApplication, type SavingsGoal } from "@/lib/supabase";
...
export default function CASPlatformControls({ accounts, onClose }: Props) {
  const [activeSection, setActiveSection] = useState<ReviewType | "overview" | "goals">("overview");
  ...
  const handleApproveTier = async (req: TierUpgradeRequest) => {
    ...
    await supabase.from("banking_notifications").insert({
      account_id: req.account_id, target: req.account_id,
      title: "Tier Upgrade Approved 🎉",
      body: `Congratulations, ${req.account_name}! Your request to upgrade to Tier ${req.requested_tier} has been approved by BankUnited CEO Administration. Your account has been upgraded and you now have access to all Tier ${req.requested_tier} features and privileges.`,
      is_read: false,
    });
    toast.success(`${req.account_name} upgraded to Tier ${req.requested_tier}.`);
    setSelected(null); setProcessing(false); fetchAll();
  };
  ...
  const handleDeclineTier = async (req: TierUpgradeRequest) => {
    ...
    await supabase.from("banking_notifications").insert({
      ...
      body: `Dear ${req.account_name}, after careful review of your Tier ${req.requested_tier} upgrade request, BankUnited CEO Administration is unable to approve this request at this time. Reason: ${declineReason.trim()}. You may reapply with updated documentation.`,
      is_read: false,
    });
    toast.success("Tier upgrade request declined.");
    setSelected(null); setDeclineReason(""); setProcessing(false); fetchAll();
  };
  ...
  const handleApproveLoan = async (req: LoanApplication) => {
    ...
    const { error } = await supabase.from("banking_transactions").insert({
      account_id: req.account_id, type: "credit", amount: req.amount,
      description: `BankUnited Loan Disbursement — ${req.purpose}`,
      sender_name: "BankUnited", admin_override: true, custom_timestamp: new Date().toISOString(),
    });
    if (!error) {
      ...
      await supabase.from("banking_notifications").insert({
        ...
        body: `Dear ${req.account_name}, your BankUnited loan application for ${formatCurrency(req.amount, "USD")} has been approved. The funds have been disbursed to your account. Repayment Terms: ${repaymentTerms.trim()}.`,
        is_read: false,
      });
      toast.success(`Loan of ${formatCurrency(req.amount, "USD")} disbursed to ${req.account_name}.`);
    }
    setSelected(null); setRepaymentTerms(""); setProcessing(false); fetchAll();
  };
  ...
  const handleApproveCheque = async (req: ChequeRequest) => {
    ...
    await supabase.from("banking_notifications").insert({
      ...
      body: `Dear ${req.account_name}, your BankUnited cheque book request has been approved. Your cheque book will be dispatched to your registered address within 5–7 business days.`,
      is_read: false,
    });
  ...
  const handleDeclineCheque = async (req: ChequeRequest) => {
    ...
    await supabase.from("banking_notifications").insert({
      ...
      body: `Dear ${req.account_name}, your BankUnited cheque book request could not be processed at this time. Reason: ${declineReason.trim()}.`,
      is_read: false,
    });
  ...
  const handleAlterTier = async (accountId: string, accountName: string, newTier: number) => {
    ...
    await supabase.from("banking_notifications").insert({
      account_id: accountId, target: accountId,
      title: "Account Tier Updated",
      body: `Your BankUnited account has been updated to Tier ${newTier} by CEO Administration.`,
      is_read: false,
    });
  ...
  return (
    <div className="fixed inset-0 z-[55] flex flex-col" style={{ background: "hsl(220,45%,8%)" }}>
      <div className="flex items-center gap-3 px-5 pt-12 pb-4 flex-shrink-0" style={{ background: "hsl(220,55%,12%)", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
        ...
        <div className="text-white font-bold flex-1">BankUnited Platform Controls</div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
        {activeSection === "overview" && (
          <>
            ...
            {[
              ...
              <button key={item.label} onClick={item.onClick} className="w-full flex items-center gap-3 p-4 rounded-2xl text-left" style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)" }}>
                <div className="w-10 h-10 rounded-2xl flex items-center justify-center flex-shrink-0" style={{ background: "rgba(200,155,50,0.12)", color: "hsl(43,85%,60%)" }}>{item.icon}</div>
                <div className="flex-1"><div className="text-white font-semibold text-sm">{item.label}</div><div className="text-white/40 text-xs">{item.count} total</div></div>
                {item.pending > 0 && <span className="bg-yellow-500 text-gray-900 text-xs rounded-full px-2 py-0.5 font-bold">{item.pending}</span>}
              </button>
            ))}
            <div className="mt-4">
              ...
              <div className="space-y-2">
                {accounts.slice(0, 20).map(acc => (
                  <div key={acc.id} className="flex items-center gap-3 p-3 rounded-2xl" style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.06)" }}>
                    ...
                    <select className="text-xs rounded-xl px-2 py-1.5 font-semibold" style={{ background: "hsl(220,50%,20%)", color: "hsl(43,85%,60%)", border: "1px solid rgba(200,155,50,0.3)" }}
                      value={acc.account_tier || 1} onChange={e => handleAlterTier(acc.id, acc.account_name, parseInt(e.target.value))}>
                      {[1, 2, 3, 4, 5].map(t => <option key={t} value={t}>Tier {t} — {TIER_NAMES[t]}</option>)}
                    </select>
                  </div>
                ))}
              </div>
        ...
        {activeSection === "tier" && !selected && (
          <>
            ...
            {tierRequests.length === 0 ? <div className="text-center py-10 text-white/25 text-sm">No tier upgrade requests</div> :
              tierRequests.map(req => (
                <button key={req.id} onClick={() => setSelected(req)} className="w-full text-left rounded-2xl p-4 space-y-1"
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span className={`text-xs px-2 py-0.5 rounded-full font-semibold ${req.status === "pending" ? "bg-yellow-500/15 text-yellow-400" : req.status === "approved" ? "bg-green-500/15 text-green-400" : "bg-red-500/15 text-red-400"}`}>{req.status}</span>
                  </div>
        ...
        {activeSection === "loan" && !selected && (
          <>
            ...
            {loanApplications.length === 0 ? <div className="text-center py-10 text-white/25 text-sm">No loan applications</div> :
        ...
        {activeSection === "cheque" && !selected && (
          <>
            ...
            {chequeRequests.length === 0 ? <div className="text-center py-10 text-white/25 text-sm">No cheque book requests</div> :
        ...
        {activeSection === "goals" && (
          <>
            ...
            {savingsGoals.length === 0 ? <div className="text-center py-10 text-white/25 text-sm">No savings goals created yet</div> :
                ...
                return (
                  <div key={goal.id} className="rounded-2xl p-4 space-y-2" style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.07)" }}>
                    <div className="flex items-start justify-between">
                      ...
                      <button onClick={() => handleToggleSavingsGoal(goal)} className="text-xs px-2.5 py-1 rounded-xl font-medium"
          ...
          return (
            <div className="space-y-3">
              ...
              <div className="rounded-3xl p-5 space-y-2" style={{ background: "hsl(220,50%,14%)", border: "1px solid rgba(255,255,255,0.08)" }}>
                ...
                {isTier && (
                  <>
                    ...
                    {(selected as TierUpgradeRequest).id_document_url && (
                      <a href={(selected as TierUpgradeRequest).id_document_url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 text-xs px-3 py-1.5 rounded-xl" style={{ background: "rgba(200,155,50,0.1)", color: "hsl(43,85%,60%)" }}>
                ...
                {isCheque && <div className="text-white/60 text-sm">Delivery: {(selected as ChequeRequest).delivery_address || "Not specified"}</div>}
              ...
              {selected.status === "pending" && (
                <div className="space-y-3">
                  {isLoan && (
                    <div>
                      <label className="text-white/60 text-xs mb-1 block">Repayment Terms (required)</label>
                      <textarea className="dark-input resize-none text-sm w-full" rows={2} placeholder="e.g. 12 monthly installments of $500…" value={repaymentTerms} onChange={e => setRepaymentTerms(e.target.value)} />
                    </div>
                  )}
                  <div className="grid grid-cols-2 gap-3">
                    <div className="space-y-2">
                      <textarea className="dark-input resize-none text-xs w-full" rows={3} placeholder="Decline reason…" value={declineReason} onChange={e => setDeclineReason(e.target.value)} />
                      <button onClick={() => { if (isTier) handleDeclineTier(selected as TierUpgradeRequest); else if (isLoan) handleDeclineLoan(selected as LoanApplication); else handleDeclineCheque(selected as ChequeRequest); }}
                        disabled={processing || !declineReason.trim()} className="w-full py-2.5 rounded-xl text-xs font-semibold"
                        style={{ background: "rgba(239,68,68,0.15)", color: "#f87171", opacity: !declineReason.trim() ? 0.5 : 1 }}>Decline</button>
                    </div>
                    <button onClick={() => { if (isTier) handleApproveTier(selected as TierUpgradeRequest); else if (isLoan) handleApproveLoan(selected as LoanApplication); else handleApproveCheque(selected as ChequeRequest); }}
              ...
              {selected.status !== "pending" && <div className="text-center py-4 text-white/30 text-sm">This request has already been {selected.status}.</div>}
            </div>
          );
        })()}
      </div>
