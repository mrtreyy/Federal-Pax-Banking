        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-4">
          ...
          <div className="rounded-3xl p-5 space-y-3" style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.08)" }}>
            <div className="text-white/50 text-sm font-semibold flex items-center gap-2">
              <Reply size={14} /> Respond to Administration
            </div>
            <textarea
            ...
            <div className="flex items-center gap-2">
              ...
              <button onClick={handleSendReply} disabled={sending || !replyText.trim()}
                ...
                {sending ? <div className="w-4 h-4 border-2 border-gray-900/30 border-t-gray-900 rounded-full animate-spin" /> : <><Send size={13} /> Send Response</>}
              </button>
            </div>
          </div>
        </div>
            ...
            <button key={n.id} onClick={() => { setSelected(n); if (!n.is_read) markOne(n.id); }}
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-start justify-between gap-2">
                    <div className="text-white text-sm font-semibold leading-tight truncate">{n.title}</div>
                    {!n.is_read && <div className="w-2 h-2 rounded-full flex-shrink-0 mt-1.5" style={{ background: "hsl(43,85%,60%)" }} />}
                  </div>
                  <div className="text-white/50 text-xs leading-relaxed mt-0.5 line-clamp-2">{n.body}</div>
